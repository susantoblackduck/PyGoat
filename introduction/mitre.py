...
def mitre_lab_17_api(request):
    if request.method == "POST":
        ...
        if not ip or not re.fullmatch(r"(?:localhost|(?=.{1,253}\Z)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*)|(?:\d{1,3}\.){3}\d{1,3}", ip):
            return HttpResponseBadRequest('Invalid IP or hostname')
        command = ["nmap", ip]
